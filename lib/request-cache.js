...
function RequestCache(options) {
  ...
  this.store.iterate(function(v, k) {
    if (self.ownStore || self.keyIsPrefixed(k)) {
      ...
      var age = _.has(v, 'ts') ? ((d0 - v.ts) / 60 / 60 / 24).toFixed(1) : 0;
      if (typeof vd === 'string') m = vd.length;
      else if (_.isArrayBuffer(vd)) m = vd.byteLength;
      //else console.log(v);
      self.stats.memory += m;
      if (self.options.debugCacheBoot) {
        console.log('[Teuthis] found key: ' + k + ', memory: ' + m + '/' + self.stats.memory + ', ' + typeof vd + ', ' + age + ' days');
      }
    }
  }, function() {
...
RequestCache.prototype.purgeItem = function(key, done) {
  var self = this;
  this.store.removeItem(k, function() {
    if (self && self.cacheKeys && self.cacheKeys.hasOwnProperty(k)) {
  ...
  if (this.ownStore) {
    ...
    self.store.iterate(function(v, k) {
      if (self.cacheKeys.hasOwnProperty(k) || self.keyIsPrefixed(k)) {
        self.store.removeItem(k, function() { delete self.cacheKeys[k]; });
      }
    }, function() {
      self.cacheKeys = {};
      self.stats.memory = 0;
