...
function XMLHttpRequestProxy() {
  ...
  xhr.onload = function onload () {
    ...
    if (xhr.status >= 200 && xhr.status < 300 && xhr.response) {
      if (shouldAddToCache_ === true) {
        var mangled = cachekeymangler(method_);
        if (options.debugEvents) console.log('[Teuthis] proxy-xhr-onload do-put ' + mangled + ' ' + url_);
        ...
        store.put(mangled, url_, savedResponse, function() {
  ...
  this.send = function() {
    ...
    if (shouldCache(method_, url_)) {
      var mangled = cachekeymangler(method_);
      if (options.debugCache) console.log('[Teuthis] proxy-try-cache ' + mangled + ' ' + url_);
      store.match(mangled, url_, function(key, cachedValue) {
        // hit
        self.status = +200;
        self.statusText = '200 OK';
        if (_.isFunction(self.onreadystatechange)) { self.onreadystatechange(); }
  ...
  ["addEventListener", "removeEventListener", "abort", "getAllResponseHeaders", "getResponseHeader", "overrideMimeType", "setRequestHeader"].forEach(function(item) {
    Object.defineProperty(self, item, {
